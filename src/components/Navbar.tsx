...
const Navbar = () => {
  ...
  return (
    <nav className="flex h-16 items-center justify-between p-4 font-semibold capitalize">
      ...
      <ul className="hidden items-center gap-4 sm:flex">
        {terms.map((term) => (
          <li
            ...
            className={
              term.name === "Contact"
                ? "rounded-full bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700"
                : "hover:underline"
            }
          >
      ...
      {hamOpen && (
        <ul className="z-50 sm:hidden absolute inset-0 top-16 flex h-fit flex-col items-center justify-center gap-4 bg-white p-6 font-semibold capitalize shadow-lg">
          {terms.map((term) => (
            <li
              key={term.name}
              className={
